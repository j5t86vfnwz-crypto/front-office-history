...
def main():
    ...
    bundle=json.loads(Path(args.file).read_text(encoding="utf-8"));year=(bundle.get("years") or {}).get("2026") or {};teams=year.get("teams") or {};errors=[];checks=0;league=defaultdict(list);total=0
    checks+=3
    ...
    for team in b.TEAMS:
        td=teams.get(team)
        if not td:errors.append(f"{team}: missing team pack");continue
        _,official_names=cur.nfl_membership(team);_,nfl_meta=cur.nfl_roster_metadata(team,official_names);roster=td.get("roster") or []
        expected={norm(n):n for n in official_names};actual={norm(p.get("full_name") or p.get("display_name")):p for p in roster};checks+=8+len(expected)+len(actual)*8
        ...
        if set(actual)!=set(expected):
            missing=[expected[k] for k in sorted(set(expected)-set(actual))];extra=[actual[k].get("full_name") for k in sorted(set(actual)-set(expected))]
            errors.append(f"{team}: NFL.com membership mismatch; missing={missing[:12]} extra={extra[:12]}")
        if len(nfl_meta)<max(40,int(len(expected)*.75)):errors.append(f"{team}: NFL.com position metadata only matched {len(nfl_meta)}/{len(expected)} players")
        rooms=defaultdict(list)
        for nk,player in actual.items():
            name=player.get("full_name") or expected.get(nk) or nk;league[identity(player)].append((team,name));pos=str(player.get("position") or "").upper();group=str(player.get("room_group") or "").upper()
            ...
            if str(player.get("team") or "").upper()!=b.BASE_ABBR[team]:errors.append(f"{team} {name}: embedded team {player.get('team')} does not match")
            src=nfl_meta.get(nk)
            if src:
                src_pos=b.canonical_pos(src.get("position"));site_pos=b.canonical_pos(pos)
                if src_pos=="DB":
                    if b.group_for_pos(site_pos) not in {"CB","S"}:errors.append(f"{team} {name}: position {site_pos} disagrees with NFL DB")
                elif src_pos!="OTHER" and site_pos!=src_pos:errors.append(f"{team} {name}: position {site_pos} != NFL.com {src_pos}")
            rooms[group].append(player)
        for group,players in rooms.items():
            ranks=sorted(int(p.get("room_order") or 999) for p in players);checks+=len(players)+1
            if ranks!=list(range(1,len(players)+1)):errors.append(f"{team} {group}: room ranks are not exactly sequential")
        total+=len(roster)
    for ident,items in league.items():
        checks+=1;owners=sorted({team for team,_ in items})
        if len(owners)>1:errors.append(f"same 2026 player identity {ident} appears on multiple teams: {owners}")
    lions={norm(p.get("full_name")) for p in (teams.get("Detroit Lions") or {}).get("roster",[])};texans={norm(p.get("full_name")) for p in (teams.get("Houston Texans") or {}).get("roster",[])};checks+=4
